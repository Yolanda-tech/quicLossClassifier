...
void _serialize( Json::Value& root, treeNode& tree)
{
    ...
    root["level"] = tree->_level;
    root["splitFN"] = tree->_splitFeatureName;
    root["splitFV"] = tree->_splitFeatureValue;
    root["label"] = tree->_label;
    Json::Value rootLeft,rootRight;
    if(tree->_left){
        _serialize(rootLeft,tree->_left);
        root["left"] = rootLeft;
    }
    if(tree->_right){
        _serialize(rootRight,tree->_right);
        root["right"] = rootRight;
    }
}

void DesionTree::Serialize( Json::Value& root)
{   
    if(_tree == NULL){
        return;
    }
   // serialize primitives
    root["level"] = _tree->_level;
    root["splitFN"] = _tree->_splitFeatureName;
    root["splitFV"] = _tree->_splitFeatureValue;
    root["label"] = _tree->_label;
    Json::Value rootLeft,rootRight;
    if(_tree->_left){
        _serialize(rootLeft,_tree->_left);
        root["left"] = rootLeft;
    }
    if(_tree->_right){
        _serialize(rootRight,_tree->_right);
        root["right"] = rootRight;
    }
} 
...
void _deserialize(Json::Value& root, treeNode& tree)
{
    ...
    tree = new Node();
    tree->_level = root.get("level",1).asInt();
    tree->_splitFeatureValue = root.get("splitFV",1).asDouble();
    tree->_splitFeatureName = root.get("splitFN",1).asDouble();
    tree->_label = root.get("label",1).asDouble();

    Json::Value rootLeft,rootRight;
    if(findKeyAndValue(root,"left",rootLeft)){
        if(!rootLeft.empty()){
            _deserialize(rootLeft,tree->_left);  
        }
    }
    
    if(findKeyAndValue(root,"right",rootRight)){
        if(!rootRight.empty()){
            _deserialize(rootRight,tree->_right);
        }
    }
}

void DesionTree::Deserialize( Json::Value& root )
{
    if(root.empty()){
        return;
    }
    _tree = new Node();
    _tree->_level = root.get("level",1).asInt();
    _tree->_splitFeatureValue = root.get("splitFV",1).asDouble();
    _tree->_splitFeatureName = root.get("splitFN",1).asDouble();
    _tree->_label = root.get("label",1).asDouble();
 
    Json::Value rootLeft,rootRight;
    if(findKeyAndValue(root,"left",rootLeft)){
        if(!rootLeft.empty()){
            _deserialize(rootLeft,_tree->_left);
        }
    }

    if(findKeyAndValue(root,"right",rootRight)){
        if(!rootRight.empty()){
            _deserialize(rootRight,_tree->_right);
        }
    }
}
...
void DesionTree::builtTree(Mat &mat, Row &featureName) {
    ...
    while (!treeContainer.empty()) {
        ...
        if (idLabelTheSame(currentMat)) {
            ...
            // Use gini gain to get the best split feature and its split value. !!!CORE!!!
...
ElementType DesionTree::classify(Row &row, map<ElementType, int> &featureIndex) {
    ...
    // created is wrong.
    cout<<"\t\tError appear! The node out of tree when classify"<<endl;
