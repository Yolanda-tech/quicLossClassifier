...
def writeToFile(path,data):
    filePath = os.path.join(path, OutFileName)
...
def traverseFile(path):    
    ...
    for file in files: 
        ...
        if not os.path.isdir(filePath):#skip subdir
            #print filePath
            if ".csv" in file:#skip the result file
                continue
            
            avgPLT[file] = fileProcess(filePath,file)
    #print avgPLT
    writeToFile(path,avgPLT)
    return avgPLT

def combineAllDirPLT(PLTdict):
    #print PLTdict
...
def getSubDirAndFile(currentDir):
    ...
    for path,dirList,fileList in subDirAndFile:  
        ...
        for dirName in dirList:
            ...
            #print subDirPath
            eachAvgPLT[dirName] = traverseFile(subDirPath)
        break #skip the subdir
    #print eachAvgPLT
    keys, combAvgPLT = combineAllDirPLT(eachAvgPLT)
    print "the list of dir:", keys
    writeToFile(currentDir,combAvgPLT)
